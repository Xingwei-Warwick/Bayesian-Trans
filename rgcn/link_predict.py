...
from .base_model import BaseRGCN
from . import utils


class EmbeddingLayer(nn.Module):
    def __init__(self, num_nodes, h_dim, load_weight=None):
        super(EmbeddingLayer, self).__init__()
        if load_weight is None:
            self.embedding = torch.nn.Embedding(num_nodes, h_dim)
        else:
            self.embedding = torch.nn.Embedding.from_pretrained(load_weight, freeze=True)
...
class RGCN(BaseRGCN):
    def build_input_layer(self, load_weight=None):
        return EmbeddingLayer(self.num_nodes, self.h_dim, load_weight)
    ...
    def build_hidden_layer(self, idx):
        ...
        return RelGraphConv(self.h_dim, self.h_dim, self.num_rels, "bdd",
                ...
                # every relation has a weight matrix. Can we use the flatten matrix as the prior

class LinkPredict(nn.Module):
    def __init__(self, in_dim, h_dim, num_rels, num_bases=-1,
                 num_hidden_layers=1, dropout=0, use_cuda=False, 
                 reg_param=0, load_weight=None):
